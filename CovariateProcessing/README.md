...
[**02-StaticCovariates.pdf**](https://github.com/sjgillman/PS_MAMU/blob/main/CovariateProcessing/02-StaticCovariates.pdf) Script to calculate and assign static environmental covariates across the study area for both grid cells and surveyed tracklines. It uses `reduced_area.gpkg`, `Dclass_cut.gpk`, `puget_poly.gpkg`, and `all_tracks_2026_final.gpkg` located in [**GIS_COVS**](https://github.com/sjgillman/PS_MAMU/blob/main/CovariateProcessing/GIS_COVS) and `grid2km_hex.gpkg` located in [**OUTPUTS**](https://github.com/sjgillman/PS_MAMU/blob/main/CovariateProcessing/OUTPUTS). Final products are `all_static_hex.gpkg` and `all_static_track.gpkg` located in [**OUTPUTS**](https://github.com/sjgillman/PS_MAMU/blob/main/CovariateProcessing/OUTPUTS). For BPI and depth, will need to download [Bathymetric Base Map for the Salish Sea Bioregion](https://www.arcgis.com/home/item.html?id=f23310e286614728a823ae84ab865cc1) directly. Also uses files `pmep_estu_ext.gpkg`, `pmep_substrat_habitat.gpkg`, and `pmep_biotic_habitat.gpkg` from [**GIS_COVS**](https://github.com/sjgillman/PS_MAMU/blob/main/CovariateProcessing/GIS_COVS). *`pmep_biotic_habitat.gpkg` was later removed but included for sources*
